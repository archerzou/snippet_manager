...
export async function PUT(request: any) {
  try {
    ...
    if (!tagId) {
      return NextResponse.json({ message: "tag ID is required" }, {
        status: 400,
      } as any);
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!tagId) {
      return NextResponse.json({ message: "tagId is required" }, {
        status: 400,
      } as any);
    }
    ...
    return NextResponse.json({ message: "Failed to delete tag" }, {
      status: 500,
    } as any);
  }
}
